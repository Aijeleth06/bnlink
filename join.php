...
$query = mysqli_query($con,"insert into user(`email`,`password`,`mobile`,`Homeaddress`,`account`,`under_userid`,`side`) values('$email','$password','$mobile','$address','$account','$under_userid','$side')");
//Insert into Tree
...
$query = mysqli_query($con,"update pinlist set status='close' where pin='$pin'");
//Inset into Icome
$query = mysqli_query($con,"insert into income (`userid`) values('$email')");

$query = mysqli_query($con,"update income set day_bal=day_bal+4,current_bal=current_bal+4,total_bal=total_bal+4 where userid='$sponsorID'");
/*$sql = "SELECT * FROM income WHERE userid = '$sponsorID'";
$result = mysqli_query($con,$sql);
while($row = mysqli_fetch_array($result)) {

	$UID = $row['userid'];
	$Dbal = $row['day_bal'];
	$Cbal = $row['current_bal'];
	$tbal = $row['total_bal'];
}
if($userID != ""){
	mysqli_query($con, "UPDATE income SET day_bal='$Dbal'+200 AND current_bal='$Cbal'+200 AND total_bal='$Tbal'+200 WHERE userid='$UID'");
}*/
...
$new_current_bal = $income_data['current_bal']+4;
$new_total_bal = $income_data['total_bal']+4;
//update income
...
global $con,$userid;
$query =mysqli_query($con,"select * from pinlist where pin='$pin' and userid='$userid' and status='open'");
...
<input type="email" name="sponsorID" class="form-control" value="<?php echo $_SESSION['userid']; ?>" required>
</div>
<div class="form-group">
<label>Pin</label>
